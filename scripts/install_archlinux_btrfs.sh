...
sgdisk --new=1:0:+${EFI_SIZE} --typecode=1:ef00 --change-name=1:${EFI_PARTLABEL} \
       --new=2:0:0            --typecode=2:8300 --change-name=2:${ROOT_PARTLABEL} /dev/${DRIVE}
...
mount -t btrfs -o x-mount.mkdir,noatime,subvol=@swap                     PARTLABEL=${ROOT_PARTLABEL} /mnt/swap
...
arch-chroot /mnt efibootmgr --create \
                            ...
                            --unicode "root=PARTLABEL=${ROOT_PARTLABEL} rootflags=${BTRFS_MOUNT_OPTIONS},subvol=@ resume=PARTLABEL=${ROOT_PARTLABEL} resume_offset=$(btrfs inspect-internal map-swapfile -r /mnt/swap/swapfile) initrd=\intel-ucode.img initrd=\initramfs-linux.img ${KERNEL_PARAMETERS}" \
                            --verbose
